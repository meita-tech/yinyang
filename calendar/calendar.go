...
func ReadFile(fn string) []string {
	...
	if err != nil {
		logrus.Fatalf("open file failure|%s|%s", fn, err)
	}
	content := string(bs)
	return strings.Split(content, "\n")
}

var yCalendar *YinCalendar

func GetCalendar() *YinCalendar {
	return yCalendar
}
...
func ParseRawData(dir string) {
	...
	days := []*Day{}
	year2Detail := map[int]*YinYear{}
	year2Months := map[int][]*YinMonth{}
	ym2Days := map[string][]*Day{}
	yang2Yin := map[string]*Day{}
	yin2Yang := map[string]*Day{}
	for _, row := range data {
		...
		if len(months) == 0 {
			year2Months[yin.Year] = append(year2Months[yin.Year], &YinMonth{Num: yin.Month, Leap: leapMonth})
		} else {
...
func NewYang(raw string) *YangDay {
	var year, month, day int
	if _, err := fmt.Sscanf(raw, "%d年%d月%d日", &year, &month, &day); err != nil {
		logrus.Fatalf("wrong yang fomat|%s", raw)
	}
...
func NewWeekDay(raw string) int {
	...
	case "星期日":
		return 7
	default:
		logrus.Fatalf("invalid week day string|%s", raw)
		return -1
	}
}
...
func NewYinDay(yinYear *int, yinMonth *int, leapMonth *bool, raw string) *YinDay {
	day := 0
	...
	if !exist {
		logrus.Fatalf("wrong yin day|%s", raw)
	}
	*yinMonth = month
