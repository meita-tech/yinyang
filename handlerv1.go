...
// ListYearH 返回年份数据
func ListYearH(c *gin.Context) {
	yc := calendar.GetCalendar()
	years := []*calendar.YinYear{}
	for y := calendar.YearStart; y < calendar.YearEnd; y++ {
		years = append(years, yc.Year2Detail[y])
	}
	logrus.WithField("client_ip", c.ClientIP()).Debugf("list year success")
	cbl.SuccessResponse(c, years)
}

func ListYearMonthH(c *gin.Context) {
	year64, err := strconv.ParseInt(c.Param("year"), 10, 32)
	if err != nil {
		logrus.WithField("client_ip", c.ClientIP()).Warnf("list year month invalid params|%s", err)
	...
	if year < calendar.YearStart || year >= calendar.YearEnd {
		logrus.WithField("client_ip", c.ClientIP()).Warnf("list year month out of range|%d", year)
		cbl.ErrorResponse(c, cbl.ErrBadRequest)
		return
	}
	...
	logrus.WithField("client_ip", c.ClientIP()).Infof("list year month success|%d", year)
	cbl.SuccessResponse(c, months)
}

func ListYearMonthDayH(c *gin.Context) {
	year := c.Param("year")
	month := c.Param("month")
	leap := c.Param("leap")
	...
	if !ok {
		logrus.WithField("client_ip", c.ClientIP()).Warnf("list year month day out of range|%s", ym)
	...
	logrus.WithField("client_ip", c.ClientIP()).Infof("list year month day success|%s", ym)
	cbl.SuccessResponse(c, days)
}

func ConvertYang2YinH(c *gin.Context) {
	year := c.Param("year")
	month := c.Param("month")
	day := c.Param("day")
	...
	if !ok {
		logrus.WithField("client_ip", c.ClientIP()).Warnf("convert yang to yin out of range|%s", ymd)
		cbl.ErrorResponse(c, cbl.ErrOutOfRange)
		return
	}
	...
	logrus.WithField("client_ip", c.ClientIP()).Infof("convert yang to yin success|%s", ymd)
	cbl.SuccessResponse(c, &resp)
}

func ConvertYin2YangH(c *gin.Context) {
	year := c.Param("year")
	month := c.Param("month")
	leap := c.Param("leap")
	day := c.Param("day")
	...
	if !ok {
		logrus.WithField("client_ip", c.ClientIP()).Warnf("convert yin to yang out of range|%s", ymd)
		cbl.ErrorResponse(c, cbl.ErrOutOfRange)
		return
	}

	logrus.WithField("client_ip", c.ClientIP()).Infof("convert yin to yang success|%s", ymd)
